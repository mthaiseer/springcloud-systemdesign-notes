...
# APPENDIX A : VISUAL STL MENTAL MODELS

## A.1 Container Selection Highway
...
```text
   ...
   +------+-------+                 +---------+----------+
   |              |                 |                    |
Need Order?    Need Lookup?     Need Order?        Need Best?
   |              |                 |                    |
vector+sort   unordered_map      set/map         priority_queue
                                     |
                               Need Duplicates?
                                     |
                                 multiset
...
## A.4 lower_bound vs upper_bound
...
# APPENDIX B : CONTEST MICRO EXAMPLES

## Example 1
Need first value >= x repeatedly
→ set.lower_bound()

## Example 2
Need count of equal prefixes
→ unordered_map<prefix,count>

## Example 3
Need largest in every window
→ deque<int> storing indices

## Example 4
Need merge overlapping meetings
