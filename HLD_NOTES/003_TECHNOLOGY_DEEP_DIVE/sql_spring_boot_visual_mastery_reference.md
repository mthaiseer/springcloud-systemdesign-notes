...
> Visual-first notes for learning SQL from basics to advanced, with Spring Boot examples, diagrams, and bottom-up query building.
...
```mermaid
flowchart LR
    Table["users table"] --> Columns["id, username"]
...
```mermaid
flowchart TD
    CTE1["WITH like_counts AS ..."] --> Main["Main query"]
    CTE2["WITH comment_counts AS ..."] --> Main
```
