...
```mermaid
flowchart TD
    User["User Action"] --> SetState["setState"]
    SetState --> React["React Re-render"]
    React --> UpdatedUI["Updated UI"]
```
...
flowchart TD
    ...
    Paint --> Effect["Run useEffect"]
    Effect --> Cleanup["Cleanup Old Effect"]
```
...
```mermaid
flowchart LR
    Ref["useRef"] --> DOM["DOM Element"]
    DOM --> Action["Focus or Access"]
```
...
```mermaid
flowchart TD
    Component["Component"] --> Query["useQuery"]
    Query --> Cache{"Cache Exists?"}
    Cache -->|Yes| Cached["Return Cached Data"]
    Cache -->|No| API["Fetch API"]
