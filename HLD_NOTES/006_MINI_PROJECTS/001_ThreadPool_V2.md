...
```java
    // Shared shutdown flag visible across all worker threads
...
> Added inline Java comments throughout the code to explain:
>
> - worker lifecycle
> - queue operations
> - shutdown logic
...
> - metrics tracking
>
> All original content and structure remain unchanged.
...
public class MiniThreadPool {
                ...
                while (running || !taskQueue.isEmpty()) {
                    try {
                        ...
                        Runnable task = taskQueue.take();
                                                // Execute actual business logic submitted by caller
                        task.run();
    ...
    public void shutdown() {
        ...
        for (Thread worker : workers) {
                        // Interrupt sleeping/waiting worker so it can stop
            worker.interrupt();
        }
    }
}
...
public class MiniThreadPool {
    ...
    public void submit(Runnable task) {
        ...
        boolean accepted =         // Non-blocking queue insertion
        // Returns false if queue is already full
        taskQueue.offer(task);
    ...
    public MiniThreadPool(int numberOfThreads, int queueCapacity) {
        ...
        for (int i = 0; i < numberOfThreads; i++) {
            Thread worker = new Thread(() -> {
                while (running || !taskQueue.isEmpty()) {
                    try {
                        Runnable task =                         // Wait for task for maximum 200ms
...
public class MiniThreadPool {
    ...
    public List<Runnable> shutdownNow() {
        ...
        List<Runnable> remainingTasks = new ArrayList<>();
                // Remove all queued but not yet executed tasks
    ...
    void putTaskBlocking(Runnable task) throws InterruptedException {
        if (!running) {
            throw new IllegalStateException("ThreadPool is not running");
                ...
                // Blocking insert
        // Producer waits until queue has free space
        taskQueue.put(task);
    }
...
public class MiniFuture<T> {
    ...
    public synchronized T get() throws Exception {
        while (!done) {
                        // Current thread waits until worker completes the task
            wait();
        }
    ...
    synchronized void complete(T result) {
        ...
        this.result = result;
        this.done = true;
                // Wake all threads waiting in get()
        notifyAll();
    }
    ...
    public void run() {
        try {
            T result = callable.call();
                        // Store result and wake threads waiting on future.get()
...
public class ThreadPoolMetrics {
    ...
    void incrementActiveWorkers() {
                // One more worker became busy
        activeWorkerCount.incrementAndGet();
    }

    void decrementActiveWorkers() {
                // Worker finished processing current task
...
if (accepted) {
                // Track successful task submission
            metrics.incrementSubmitted();
} else {
                // Track overload/backpressure rejection
...
public class MiniScheduledThreadPool {
    ...
    public void schedule(Runnable task, long delayMillis) {
        ...
        long runAt = System.currentTimeMillis() + delayMillis;
                // Add task into delayed priority queue sorted by execution time
    ...
    private void schedulerLoop() {
        while (running || !delayedQueue.isEmpty()) {
            try {
                                // Check earliest scheduled task without removing it
                ...
                if (next.runAtMillis <= now) {
                    delayedQueue.poll();
                                        // Delegate actual execution to worker thread pool
