...
# MiniRedis Phase 17 — Hash Data Type
...
```text
Add Redis Hash commands for object-like field storage.
```
...
```text
String keys could store only one flat value.
```
...
```text
We add HSET, HGET, HDEL, and HGETALL.
```
...
```bash
java -cp out com.miniredis.driver.Phase017Driver
```
...
```text
Nested HashMap, object fields, O(1) field lookup
```
...
```text
user profiles, product metadata, session attributes
```
