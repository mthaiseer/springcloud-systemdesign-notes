...
```text
Add channel-based publish/subscribe messaging.
```
...
```text
Clients could only request data; server could not fan out messages.
```
...
```text
We add SUBSCRIBE and PUBLISH with channel subscriber registry.
```
...
```text
Observer pattern, fanout list, queues
```
...
```text
notifications, chat, live updates, invalidation events
```
