...
# MiniRedis Phase 24 — Distributed Sharding
...
```text
Route Redis commands to the correct shard.
```
...
```text
Consistent hashing mapped keys to nodes, but there was no cluster client to route operations.
```
...
```text
We add ClusterClient that picks shard by key and sends command there.
```

Commands or operations covered:

```text
SET user:1 x routed to shard-2
```
...
```bash
java -cp out com.miniredis.driver.Phase024Driver
```
...
```text
Partition routing, ownership, shard map
```
...
```text
distributed cache, horizontal scaling, multi-node storage
```
