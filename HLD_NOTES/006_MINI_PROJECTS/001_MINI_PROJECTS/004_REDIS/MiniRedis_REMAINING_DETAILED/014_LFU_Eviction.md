...
# MiniRedis Phase 14 — LFU Eviction
...
```text
Add least-frequently-used eviction.
```
...
```text
LRU keeps recently accessed keys but may evict frequently useful keys that were not accessed recently.
```
...
```text
We add frequency counters and evict the key with lowest access count.
```
...
```bash
java -cp out com.miniredis.driver.Phase014Driver
```
...
```text
Frequency map, min-frequency tracking, tie-breaking
```
...
```text
hot-key protection, intelligent caching, memory-aware Redis config
```
