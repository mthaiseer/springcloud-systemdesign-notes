...
# MiniRedis Phase 7 — Thread Safe Storage
...
```text
Make the storage engine safe for multi-client access.
```
...
```text
HashMap is not safe when multiple client threads read/write at the same time.
```
...
```text
We replace HashMap with ConcurrentHashMap and use atomic operations where needed.
```
...
public class RedisStore {
    private final Map<String, ValueEntry> data = new ConcurrentHashMap<>();
...
```bash
java -cp out com.miniredis.driver.Phase007Driver
```
...
```text
ConcurrentHashMap, race conditions, atomicity, visibility
```
...
```text
multi-client Redis server, API cache under load, shared distributed state
```
