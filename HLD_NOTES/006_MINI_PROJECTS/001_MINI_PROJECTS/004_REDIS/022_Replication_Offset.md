...
# MiniRedis Phase 22 — Replication Offset
...
```text
Track replication progress with monotonically increasing offsets.
```
...
```text
Replica sync had no checkpoint or progress tracking.
```
...
```text
We attach offsets to replicated writes and let replicas acknowledge last applied offset.
```
...
```bash
java -cp out com.miniredis.driver.Phase022Driver
```
...
```text
Offset tracking, log position, checkpointing
```
...
```text
Kafka offsets, Redis replication backlog, recovery sync
```
