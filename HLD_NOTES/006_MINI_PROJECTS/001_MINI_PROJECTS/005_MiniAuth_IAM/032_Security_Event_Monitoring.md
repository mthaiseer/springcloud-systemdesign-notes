...
# MiniAuth / IAM Phase 32 — Security Event Monitoring
...
```text
Detect suspicious login/security patterns.
```
...
```text
Suspicious patterns are invisible.
```
...
## 8.1 `SecurityEventStore.java`

### Logic before this class

Many IAM features are based on temporary security events:
...
This store models those temporary events.
...
public class SecurityEventStore {
    static class Event {
        ...
        Event(String key, String value, long ttlSeconds) {
            this.key = key;
            this.value = value;
    ...
    public void add(String key, String value, long ttlSeconds) {
        events.computeIfAbsent(key, k -> new ArrayList<>()).add(new Event(key, value, ttlSeconds));
    }

    public long countActive(String key) {
        List<Event> list = events.getOrDefault(key, List.of());
        return list.stream().filter(e -> !e.expired()).count();
    }

    public boolean verifyLatest(String key, String value) {
        List<Event> list = events.getOrDefault(key, List.of());

        for (int i = list.size() - 1; i >= 0; i--) {
            Event e = list.get(i);
            if (!e.expired() && e.value.equals(value)) {
                return true;
            }
        }
...
## 8.2 `Phase032Driver.java`

### Logic before this class

The driver simulates temporary security flow.

```java
package com.miniauth.driver;

import com.miniauth.security.SecurityEventStore;

public class Phase032Driver {
    public static void main(String[] args) throws Exception {
        SecurityEventStore store = new SecurityEventStore();

        store.add("mohamed@example.com", "123456", 2);

        System.out.println("Active count = " + store.countActive("mohamed@example.com"));
        System.out.println("Verify 123456 = " + store.verifyLatest("mohamed@example.com", "123456"));

        Thread.sleep(2500);

        System.out.println("Active count after expiry = " + store.countActive("mohamed@example.com"));
    }
}
```
...
```bash
java -cp out com.miniauth.driver.Phase032Driver
```
