...
# MiniGateway Phase 5 — Least Connections Load Balancer
...
```text
Route requests to the backend with the fewest active connections.
```
...
```text
Round robin ignores backend load.
```
...
```text
choose minimum active connections
```
...
A service instance represents one backend server.

The gateway needs to know whether it is healthy and how many active requests it has.

```java
package com.minigateway.lb;
...
public class ServiceInstance {
    ...
    public void increment() { activeConnections++; }
    public void decrement() { activeConnections--; }

    @Override
    public String toString() {
        return id + "(" + host + ":" + port + ", healthy=" + healthy + ", active=" + activeConnections + ")";
    }
}
```
...
## 8.2 `LoadBalancer.java`

### Logic before this class

The load balancer picks one healthy backend.

Round robin rotates.

Least connections chooses the backend with fewer active requests.
...
public class LoadBalancer {
    ...
    public ServiceInstance roundRobin() {
        List<ServiceInstance> healthy = instances.stream()
                .filter(ServiceInstance::isHealthy)
                .toList();

        if (healthy.isEmpty()) {
            throw new IllegalStateException("No healthy instances");
        }

        int i = Math.abs(index.getAndIncrement()) % healthy.size();
        return healthy.get(i);
    }

    public ServiceInstance leastConnections() {
...
## 8.3 `Phase005Driver.java`

### Logic before this class

The driver demonstrates backend selection.

```java
package com.minigateway.driver;

import com.minigateway.lb.LoadBalancer;
import com.minigateway.lb.ServiceInstance;

import java.util.List;

public class Phase005Driver {
    public static void main(String[] args) {
        ServiceInstance a = new ServiceInstance("user-1", "localhost", 9001);
        ServiceInstance b = new ServiceInstance("user-2", "localhost", 9002);
        ServiceInstance c = new ServiceInstance("user-3", "localhost", 9003);

        b.increment();
        b.increment();

        LoadBalancer lb = new LoadBalancer(List.of(a, b, c));
        ...
        System.out.println("Least connections:");
        System.out.println(lb.leastConnections());

        a.setHealthy(false);
        c.setHealthy(false);

        System.out.println("After health changes:");
        System.out.println(lb.roundRobin());
    }
}
```
...
```bash
java -cp out com.minigateway.driver.Phase005Driver
```
...
```text
Client sends request related to: Least Connections Load Balancer
```
...
3. Gateway applies LeastConnectionsLoadBalancer.
