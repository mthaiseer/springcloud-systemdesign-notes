...
# MiniGateway Phase 21 — Circuit Breaker
...
```text
Stop sending traffic to failing backends temporarily.
```
...
```text
Gateway keeps calling failing backend.
```
...
```bash
java -cp out com.minigateway.driver.Phase021Driver
```
...
```text
Client sends request related to: Circuit Breaker
```
...
3. Gateway applies CircuitBreaker.
