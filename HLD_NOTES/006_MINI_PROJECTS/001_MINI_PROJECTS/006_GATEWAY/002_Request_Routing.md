...
# MiniGateway Phase 2 — Request Routing
...
```text
Add route table so gateway can map requests to different backend services.
```
...
```text
All requests go to one hardcoded backend.
```
...
```bash
java -cp out com.minigateway.driver.Phase002Driver
```
...
```text
Client sends request related to: Request Routing
```
...
```text
HashMap lookup, prefix matching
```
