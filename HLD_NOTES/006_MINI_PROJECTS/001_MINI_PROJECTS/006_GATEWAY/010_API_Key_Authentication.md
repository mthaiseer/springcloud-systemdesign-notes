...
# MiniGateway Phase 10 — API Key Authentication
...
```text
Authenticate machine clients using API keys.
```
...
```text
Machine clients need non-user auth.
```
...
```bash
java -cp out com.minigateway.driver.Phase010Driver
```
...
```text
Client sends request related to: API Key Authentication
```
...
3. Gateway applies ApiKeyFilter.
