...
# MiniGateway Phase 4 — Load Balancer Round Robin
...
```text
Distribute requests across multiple backend instances using round-robin.
```
...
```text
One backend instance receives all traffic.
```
...
```bash
java -cp out com.minigateway.driver.Phase004Driver
```
...
```text
Client sends request related to: Load Balancer Round Robin
```
...
3. Gateway applies RoundRobinLoadBalancer.
...
```text
Modulo arithmetic, circular array
```
