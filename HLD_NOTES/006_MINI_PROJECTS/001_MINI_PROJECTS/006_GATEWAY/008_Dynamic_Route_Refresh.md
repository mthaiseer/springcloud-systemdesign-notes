...
# MiniGateway Phase 8 — Dynamic Route Refresh
...
```text
Refresh route table without restarting the gateway.
```
...
```text
DynamicRouteRefresher
```

Core flow:

```text
refresh config atomically
```
...
```bash
java -cp out com.minigateway.driver.Phase008Driver
```
...
```text
Client sends request related to: Dynamic Route Refresh
```
...
3. Gateway applies DynamicRouteRefresher.
