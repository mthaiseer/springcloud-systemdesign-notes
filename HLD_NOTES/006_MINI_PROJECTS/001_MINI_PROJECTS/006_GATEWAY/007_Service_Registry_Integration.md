...
# MiniGateway Phase 7 — Service Registry Integration
...
```text
Discover backend instances dynamically from a registry.
```
...
```text
Backends are statically configured.
```
...
```bash
java -cp out com.minigateway.driver.Phase007Driver
```
...
```text
Client sends request related to: Service Registry Integration
```
...
3. Gateway applies ServiceRegistry.
