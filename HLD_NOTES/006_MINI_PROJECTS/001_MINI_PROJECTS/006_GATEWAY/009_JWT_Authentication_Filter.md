...
# MiniGateway Phase 9 — JWT Authentication Filter
...
```text
Verify JWT access tokens before forwarding requests.
```
...
```text
Authorization header -> verified user
```
...
```bash
java -cp out com.minigateway.driver.Phase009Driver
```
...
```text
Client sends request related to: JWT Authentication Filter
```
...
3. Gateway applies JwtAuthenticationFilter.
