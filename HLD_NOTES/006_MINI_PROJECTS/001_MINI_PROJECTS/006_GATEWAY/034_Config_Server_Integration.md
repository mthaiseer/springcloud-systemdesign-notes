...
# MiniGateway Phase 34 — Config Server Integration
...
```text
Load gateway routes and policies from centralized config.
```
...
```bash
java -cp out com.minigateway.driver.Phase034Driver
```
...
```text
Client sends request related to: Config Server Integration
```
...
3. Gateway applies ConfigServerClient.
