...
# MiniGateway Phase 3 — Path Based Routing
...
```text
Route traffic based on URL path prefixes like /users, /orders, /payments.
```
...
```bash
java -cp out com.minigateway.driver.Phase003Driver
```
...
```text
Client sends request related to: Path Based Routing
```
...
3. Gateway applies PathRouteMatcher.
