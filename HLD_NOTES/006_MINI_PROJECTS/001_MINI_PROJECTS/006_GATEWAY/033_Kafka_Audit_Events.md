...
# MiniGateway Phase 33 — Kafka Audit Events
...
```text
Publish gateway audit events to Kafka.
```
...
```text
Security events are not captured centrally.
```
...
```bash
java -cp out com.minigateway.driver.Phase033Driver
```
...
```text
Client sends request related to: Kafka Audit Events
```
...
3. Gateway applies AuditEventPublisher.
