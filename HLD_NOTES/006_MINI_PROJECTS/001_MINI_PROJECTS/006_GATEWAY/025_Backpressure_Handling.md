...
# MiniGateway Phase 25 — Backpressure Handling
...
```text
Reject or slow traffic when gateway/backend is overloaded.
```
...
```text
Gateway accepts more than it can process.
```
...
```bash
java -cp out com.minigateway.driver.Phase025Driver
```
...
```text
Client sends request related to: Backpressure Handling
```
...
3. Gateway applies BackpressureFilter.
