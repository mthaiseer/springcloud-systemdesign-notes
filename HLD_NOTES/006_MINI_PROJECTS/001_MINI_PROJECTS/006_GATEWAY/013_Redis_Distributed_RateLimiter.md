...
# MiniGateway Phase 13 — Redis Distributed RateLimiter
...
```text
Redis Distributed RateLimiter
```

Purpose:

```text
Make rate limiting work across multiple gateway instances.
```
...
```text
Local rate limiter fails across multiple gateways.
```
...
```bash
java -cp out com.minigateway.driver.Phase013Driver
```
...
```text
Client sends request related to: Redis Distributed RateLimiter
```
...
3. Gateway applies DistributedRateLimiter.
