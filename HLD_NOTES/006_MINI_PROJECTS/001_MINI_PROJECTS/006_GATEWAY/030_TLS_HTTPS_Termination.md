...
# MiniGateway Phase 30 — TLS HTTPS Termination
...
```text
Terminate HTTPS at gateway and forward HTTP internally.
```
...
```bash
java -cp out com.minigateway.driver.Phase030Driver
```
...
```text
Client sends request related to: TLS HTTPS Termination
```
...
3. Gateway applies TlsTermination.
