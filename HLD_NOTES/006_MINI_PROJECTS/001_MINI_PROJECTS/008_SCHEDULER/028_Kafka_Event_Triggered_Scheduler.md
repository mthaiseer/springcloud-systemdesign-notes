...
# MiniScheduler Phase 28 — Kafka Event Triggered Scheduler
...
```text
Kafka Event Triggered Scheduler
```

Purpose:

```text
Schedule tasks from events.
```
...
## 8.1 `WorkflowTask.java`

### Logic before this class

Workflow scheduling needs dependency tracking.

A task can run only when its dependencies are complete.

```java
package com.minischeduler.workflow;
...
public class WorkflowTask {
    ...
    public void dependsOn(String taskId) {
...
## 8.2 `DagScheduler.java`

### Logic before this class

The DAG scheduler runs tasks whose dependencies are complete.

This is the basis of Airflow/Temporal-style workflow thinking.
...
public class DagScheduler {
    ...
    private final Map<String, WorkflowTask> tasks = new HashMap<>();
    ...
    public void tick() {
        for (WorkflowTask task : tasks.values()) {
            if (!task.isCompleted() && task.isReady(completed)) {
                System.out.println("Executing workflow task: " + task.getId());
                task.complete();
...
import com.minischeduler.workflow.DagScheduler;
import com.minischeduler.workflow.WorkflowTask;

public class PhaseDriver {

    public static void main(String[] args) {
        WorkflowTask a = new WorkflowTask("A");
        WorkflowTask b = new WorkflowTask("B");
        WorkflowTask c = new WorkflowTask("C");
        ...
        System.out.println("Completed = " + scheduler.completedTasks());
    }
}
```
