...
# MiniRedis Phase 5 — TTL Expiration
...
```text
Add expiring keys using SET key value EX seconds and TTL key.
```
...
```text
Keys lived forever. That is not realistic for sessions, OTPs, locks, rate limit windows, or temporary cache data.
```
...
```text
We wrap values in ValueEntry with expireAtMillis and implement lazy expiration during GET, EXISTS, and TTL.
```
...
```bash
java -cp out com.miniredis.driver.Phase005Driver
```
...
```text
Timestamp comparison, lazy deletion, metadata wrapper, O(1) access
```
...
```text
sessions, OTP, idempotency keys, rate limiter windows, temporary locks
```
