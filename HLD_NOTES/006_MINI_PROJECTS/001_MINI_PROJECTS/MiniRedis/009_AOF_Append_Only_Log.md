...
# MiniRedis Phase 009 — AOF Append Only Log
...
Record every write command into append-only log.
...
Snapshot can lose recent writes.
...
We append SET/DEL commands to a log file before/after applying writes.
...
/*
Logic before class:
Driver proves phase 009 behavior without telnet or external tools.
*/
...
java -cp out com.miniredis.phase009.Phase009Driver
```
...
```text
Input operation for AOF Append Only Log
```
...
After : new AOF Append Only Log behavior available
```
