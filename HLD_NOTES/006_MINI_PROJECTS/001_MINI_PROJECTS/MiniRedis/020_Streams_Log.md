...
# MiniRedis Phase 020 — Streams Log
...
Implement append-only stream log.
...
Pub/Sub loses messages if offline.
...
We add XADD and XREAD with monotonically increasing IDs.
...
/*
Logic before class:
MessageBus supports volatile Pub/Sub and durable stream-style storage.
*/
class MessageBus {
    private final Map<String, List<String>> channels = new HashMap<>();
    private final List<String> stream = new ArrayList<>();

    void subscribe(String channel, String clientId) {
        channels.computeIfAbsent(channel, c -> new ArrayList<>()).add(clientId);
    }

    void publish(String channel, String message) {
        for (String client : channels.getOrDefault(channel, List.of())) {
            System.out.println("deliver to " + client + ": " + message);
        }
    }

    String xadd(String message) {
        String id = System.currentTimeMillis() + "-" + stream.size();
        stream.add(id + " " + message);
        return id;
    }
...
/*
Logic before class:
Driver demonstrates notification fanout and stream replay.
*/
public class Phase020Driver {
    public static void main(String[] args) {
        ...
        String id = bus.xadd("payment-success");
        System.out.println("XADD id=" + id);
        System.out.println(bus.xread());
    }
}
```
...
java -cp out com.miniredis.phase020.Phase020Driver
```
...
```text
Input operation for Streams Log
```
...
After : new Streams Log behavior available
```
