...
# MiniRedis Phase 003 — In-Memory KeyValue Store
...
Add a HashMap-backed storage engine.
...
Parser exists but no real storage.
...
We add RedisStore with set/get/delete/exists methods.
...
Input
  -> Validate
  -> Execute In-Memory KeyValue Store
...
/*
Logic before class:
Driver proves phase 003 behavior without telnet or external tools.
*/
...
java -cp out com.miniredis.phase003.Phase003Driver
```
...
```text
Input operation for In-Memory KeyValue Store
```
...
After : new In-Memory KeyValue Store behavior available
```
