...
# MiniRedis Phase 022 — Replication Offset
...
Track replicated progress with offsets.
...
Replica sync has no checkpoint.
...
We add offset to each write and replica acknowledgements.
...
java -cp out com.miniredis.phase022.Phase022Driver
```
...
```text
Input operation for Replication Offset
```
...
After : new Replication Offset behavior available
```
