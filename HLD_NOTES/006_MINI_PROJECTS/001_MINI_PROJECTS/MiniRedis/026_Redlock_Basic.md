...
# MiniRedis Phase 026 — Redlock Basic
...
Acquire lock across multiple Redis nodes.
...
Single-node lock can fail with node loss.
...
We add quorum-based lock acquisition concept.
...
```text
Quorum, distributed coordination
```
...
java -cp out com.miniredis.phase026.Phase026Driver
```
...
```text
Input operation for Redlock Basic
```
...
After : new Redlock Basic behavior available
```
