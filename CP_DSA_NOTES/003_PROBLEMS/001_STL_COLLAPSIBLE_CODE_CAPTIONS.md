# Competitive Programming STL & Problem Solving Notes — Collapsible Code Edition
...
> Clean markdown notes with compact captions and collapsible C++ code blocks.
...
> **Caption:** Build the habit of reading constraints, finding brute force, identifying bottlenecks, and replacing them with a known pattern.
...
> **Caption:** Use counter for one bracket type and stack for multiple bracket types.
...
> **Caption:** Fixed-size windows need insert current, remove outgoing, then answer.
...
> **Caption:** Use multiset for easy O(n log k), or monotonic deque for optimal O(n).
...
> **Caption:** Median minimizes sum of absolute differences; maintain it with two multisets.
...
> **Caption:** Maintain different statistics with sum, square sum, frequency map, and two multisets.
...
> **Caption:** Convert subarray sum queries into prefix-sum lookup problems.
...
> **Caption:** Monotonic stack stores only useful candidates for next greater/smaller queries.
...
> **Caption:** Use stack to find left wall, bottom, and right wall whenever current height is higher.
...
> **Caption:** Static intervals use sorted endpoints; dynamic intervals use merged interval set.
...
> **Caption:** Maintain largest k values in one multiset and the rest in another; keep sum of top k.
...
> **Caption:** Use heaps when you repeatedly need the current best element.
...
> **Caption:** Stack is LIFO, queue is FIFO; both model order of processing.
...
> **Caption:** Count how much each element contributes instead of enumerating all objects.
...
> **Caption:** Treat pattern printing as coordinate geometry: decide whether cell (i,j) is star or space.
...
> **Caption:** Nested parentheses are parsed using recursion or stack, then multiplied and merged.
...
> **Caption:** Use monotonic increasing stack; each popped bar becomes the limiting height of a rectangle.
...
> **Caption:** Match operation requirements to containers: order, frequency, priority, window, or range.
...
> **Caption:** Most STL bugs come from wrong erase, empty access, iterator misuse, or missing initialization.
...
> **Caption:** Use problem clues to jump quickly to the right pattern.
...
> **Caption:** Keep a small reliable template for fast contest implementation.
...
> **Caption:** Before coding, check constraints, pattern, edge cases, and complexity.
...
> **Caption:** Think in invariants, patterns, and complexity before writing code.
