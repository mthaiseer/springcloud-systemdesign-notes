...
Goal: split array into at most `k` parts and minimize the maximum part sum.

```mermaid
flowchart LR
    A[Array] --> B[Choose max allowed sum mid]
    B --> C[Greedily make groups]
    C --> D{Groups needed less or equal k}
    D -->|yes| E[mid is possible]
    D -->|no| F[mid is too small]
```
