...
### Top-down C++ (memoization / recursive style from note)
...
public:
    int maxProduct(vector<int>& nums) {
        ...
        for (int i = 1; i < nums.size(); i++) {
            int x = nums[i];
            int nmx = max({x, x * mx, x * mn});
            int nmn = min({x, x * mx, x * mn});
            mx = nmx; mn = nmn;
            ans = max(ans, mx);
        }
        return ans;
    ...
    int uniquePathsWithObstacles(vector<vector<int>>& grid) {
        ...
        for (int r = R - 1; r >= 0; r--) {
            for (int c = C - 1; c >= 0; c--) {
                ...
                else if (r == R - 1 && c == C - 1) dp[r][c] = 1;
                else {
                    int down = (r + 1 < R) ? dp[r + 1][c] : 0;
                    int right = (c + 1 < C) ? dp[r][c + 1] : 0;
                    dp[r][c] = down + right;
                }
            }
        }
...
public:
    int minPathSum(vector<vector<int>>& grid) {
        ...
        for (int r = R - 1; r >= 0; r--) {
            for (int c = C - 1; c >= 0; c--) {
                if (r == R - 1 && c == C - 1) dp[r][c] = grid[r][c];
                else {
                    int down = (r + 1 < R) ? dp[r + 1][c] : INT_MAX / 2;
                    int right = (c + 1 < C) ? dp[r][c + 1] : INT_MAX / 2;
                    dp[r][c] = grid[r][c] + min(down, right);
...
public:
    int minCut(string s) {
        ...
        for (int i = n - 1; i >= 0; i--) {
            dp[i] = INT_MAX / 2;
            for (int j = i; j < n; j++) if (pal[i][j]) dp[i] = min(dp[i], 1 + dp[j + 1]);
        }
        return dp[0];
    }
};
    ...
    int minCost(int n, vector<int>& cuts) {
        ...
        for (int len = 2; len < m; len++) {
            for (int l = 0; l + len < m; l++) {
                ...
                dp[l][r] = INT_MAX;
                for (int k = l + 1; k < r; k++) {
                    dp[l][r] = min(dp[l][r], p[r] - p[l] + dp[l][k] + dp[k][r]);
                }
                if (dp[l][r] == INT_MAX) dp[l][r] = 0;
            }
        }
        return dp[0][m - 1];
    }
...
public:
    bool canPartitionKSubsets(vector<int>& nums, int k) {
        ...
        for (int mask = 0; mask < N; mask++) {
            ...
            for (int i = 0; i < n; i++) {
                if (mask & (1 << i)) continue;
                if (dp[mask] + nums[i] <= target) dp[mask | (1 << i)] = (dp[mask] + nums[i]) % target;
            }
        }
        return dp[N - 1] == 0;
    }
...
class Solution {
public:
    pair<int,int> postorderDP(TreeNode* node) {
        if (!node) return {0, 0};
        auto L = postorderDP(node->left);
        auto R = postorderDP(node->right);
        int skip = max(L.first, L.second) + max(R.first, R.second);
        int take = node->val + L.first + R.first;
        return {skip, take};
    }
    ...
    int rob(TreeNode* root) {
        auto [skip, take] = postorderDP(root);
...
public:
    ...
    int postorderDP(TreeNode* node) {
        if (!node) return 0;
        int L = max(0, postorderDP(node->left));
        int R = max(0, postorderDP(node->right));
...
public:
    bool isScramble(string s1, string s2) {
        ...
        for (int i = 0; i < n; i++) for (int j = 0; j < n; j++) dp[1][i][j] = (s1[i] == s2[j]);

        for (int len = 2; len <= n; len++) {
            for (int i = 0; i + len <= n; i++) {
                for (int j = 0; j + len <= n; j++) {
                    for (int cut = 1; cut < len && !dp[len][i][j]; cut++) {
                        bool noSwap = dp[cut][i][j] && dp[len - cut][i + cut][j + cut];
                        bool swap = dp[cut][i][j + len - cut] && dp[len - cut][i + cut][j];
                        dp[len][i][j] = noSwap || swap;
                    }
                }
            }
        }
